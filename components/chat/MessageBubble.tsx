...
export function MessageBubble({ msg, streaming = false }: { msg: Message; streaming?: boolean }) {
  ...
  return (
    <div className={`flex gap-3 py-1.5 animate-fadein ${isUser ? "justify-end" : ""}`}>
      {!isUser && (
        <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0 mt-0.5 text-[10px] font-bold transition-colors duration-300 ease-in-out"
      ...
      <div className={`max-w-[84%] space-y-1.5 ${isUser ? "items-end flex flex-col" : ""}`}>
        {/* Bubble */}
        <div className="rounded-2xl px-4 py-3 text-sm transition-colors duration-300 ease-in-out"
          style={isUser ? {
            background: T.accent,
            color: T.accentText,
            fontWeight: 500,
          ...
          {isUser ? (
            ...
            <div className="md-wrap transition-colors duration-300 ease-in-out" style={{ color: T.text }}>
              <ReactMarkdown remarkPlugins={[remarkGfm]}>{msg.content}</ReactMarkdown>
              {streaming && (
                <span className="inline-block w-1.5 h-[1.1em] ml-0.5 rounded-sm align-middle animate-blink transition-colors duration-300 ease-in-out"
        ...
        {!isUser && msg.sources && msg.sources.length > 0 && (
          <div className="flex flex-wrap gap-1.5 px-1">
            {msg.sources.map(src => (
              <span key={src} className="text-[10px] px-2 py-0.5 rounded-full transition-colors duration-300 ease-in-out"
